...
const server = http.createServer((req, res) => {
  ...
  // Simple backend listing (mimic production API for e2e tests)
  if (req.method === 'GET' && url === '/backends') {
    res.writeHead(200, { 'Content-Type': 'application/json' })
    res.end(JSON.stringify([
      { name: 'dummy', label: 'Dummy Backend' },
      { name: 'meep', label: 'Meep (FDTD)' },
      { name: 'synthesis', label: 'Synthesis (bundles)' }
    ]))
    return
  }

  if (req.method === 'GET' && url === '/backends/synthesis') {
    res.writeHead(200, { 'Content-Type': 'application/json' })
