    ...
    resourceTimer.current = setInterval(async () => {
      ...
      if (data) {
        setResourceData((prev) => {
          // Only append if different from last entry to keep table compact
          const last = prev[prev.length - 1];
          const changed = !last ||
            last.cpu_percent !== data.cpu_percent ||
            last.memory_rss !== data.memory_rss ||
            last.memory_vms !== data.memory_vms ||
            last.memory_percent !== data.memory_percent;
          if (!changed) return prev;
          // Keep recent history bounded (for graph performance)
